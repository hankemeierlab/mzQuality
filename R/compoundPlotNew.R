...
addTextBubble <- function(exp, df, type = "aliquot") {
    if (type == "aliquot") {
        ...
        df$text <- vapply(1:nrow(metadata), function(i){
            names <- c(type, colnames(metadata))
            values <- c(rownames(metadata)[i], as.vector(metadata[i, ]))
            paste0(glue::glue("{name}: {value}", name = names, value = values), collapse = "\n")
        }, character(1))
        ...
        res <- vapply(1:nrow(metadata), function(i){
            names <- c(type, colnames(metadata))
            values <- c(rownames(metadata)[i], as.vector(metadata[i, ]))
            paste0(glue::glue("{name}: {value}", name = names, value = values), collapse = "\n")
        }, character(1))
...
compoundPlotNew <- function(exp, compound = 1, batches = exp$batch, types = exp$type,
    ...
    if (addInternalStandards | assay == "area" & metadata(exp)$hasIS) {
        df_is <- as.data.frame(preparePlotData(exp[compound, ], assay = "area_is",
...
compoundPlot <- function(df, assay, trendTypes, colors, smooth = TRUE, doLog = TRUE){
    ...
    if (smooth && nrow(trendData) > 0) {
        ...
        p <- p + scale_color_manual(values = colors) +
            geom_smooth(formula = "y ~ x", inherit.aes = TRUE, method=lm, show.legend = FALSE, na.rm = TRUE,
...
violinPlotNew <- function(exp, assay = "ratio_corrected",
    ...
    if (addMedian) {
        p <- p + stat_summary(fun = "median", geom = "point", shape = 24, size = 3,
...
getPcaData <- function(exp, assay = assayNames(exp)[1],
                       ...
                       doLog = TRUE){
    df <- preparePlotData(exp = exp, assay = assay, batches = batches,
...
pcaBiPlot <- function(pc) {
    ...
    scale_factor <- max(abs(scores[, 1:2])) / max(abs(loadings[, 1:2]))
...
pcaPlotNew <- function(exp, assay = assayNames(exp)[1],
    ...
    pc1name <- glue::glue("PC{xvar} ({format(perc, digits = 4)}%)", xvar = pc1, perc = expvar[pc1] * 100)
    pc2name <- glue::glue("PC{yvar} ({format(perc, digits = 4)}%)", yvar = pc2, perc = expvar[pc2] * 100)
    ...
    loadings <- data.frame(pc$rotation, variable = rownames(pc$rotation))
    scale_factor <- max(abs(data[, 1:2])) / max(abs(loadings[, 1:2]))
    loadings[, 1:2] <- loadings[, 1:2] * scale_factor

    # Make plot
    p <- ggplot() +
        geom_point(data = data, aes(x = .data$PC1, y = .data$PC2, fill = .data$type, text = .data$text,
                             label = .data$aliquot, group = .data$type), size = 3, stroke = 0.2, shape = 21, color = "black") +
        theme_minimal() +
        xlab(pc1name) +
        ylab(pc2name) +
    ...
    if (addConfidenceInterval) {
        p <- p + stat_ellipse(data = data, level = 0.95, mapping = aes(group = .data$type, color = .data$type, x = .data$PC1, y = .data$PC2),
...
concentrationPlotNew <- function(exp, assay = "ratio_corrected", compound = 1,
    ...
    ggplot(df, aes(x = .data$concentration, y = .data$ratio, color = .data$type,
                   group = .data$batch)) +
        suppressWarnings(geom_point(size = 2, aes(text = .data$text))) +
        ...
        geom_smooth(formula = "y ~ x", method = lm, inherit.aes = withinTrend,  level = .95,
